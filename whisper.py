...
def main(model, english,verbose, energy, pause,dynamic_energy,save_file,device):
    ...
    if model != "large" and english:
        model = model + ".en"
    audio_model = whisper.load_model(model).to(device)   #_____________________  audio_model
...
def transcribe_forever(audio_queue, result_queue, audio_model, english, verbose, save_file):
    while True:
        ...
        if english:
            result = audio_model.transcribe(audio_data,language='english') #____________________audio_model
        else:
